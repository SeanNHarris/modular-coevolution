...
def merge_run_parameters(primary_run_parameters: list[dict[str, Any]], secondary_run_parameters: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Prepare to merge two lists of run parameters, such as those generated by :func:`generate_run_parameters` and :func:`run_parameters_from_logs`.
    Handles treatments by treatment string and runs by run index.
    Each run's parameters will be configured to merge in the parameters from the secondary list,
    which will only be used for entries that do not exist in the primary list.

    Note: Performing the complete merge can not be done immediately, as it must come after processing of population default parameters.
        However, the behavior of population default parameters may rely on the experiment parameters.
    ...
    Returns:
        A list of run parameters comparable to those generated by :func:`generate_run_parameters`, configured to merge the parameters from the primary and secondary lists.
    """
    ...
    for run_identifier, primary_parameters in primary_parameters_by_treatment.items():
        ...
        # Do not merge the secondary parameters immediately; this has to wait until after processing of default parameters.
        merged_parameters['merge_parameters'] = secondary_parameters

        # Experiment parameters can't be part of the population default parameters, and can be merged immediately.
        # Additionally, the set of populations may depend on the experiment parameters, so they MUST be merged now.
        if 'experiment' in merged_parameters and 'experiment' in secondary_parameters:
            deep_update_dictionary(merged_parameters['experiment'], secondary_parameters['experiment'], weak=True)
